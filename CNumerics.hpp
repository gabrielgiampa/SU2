...
public:
  ...
  */
   ...
   */
  inline virtual ResidualType<> ComputeResidual(const CConfig* config) {
    //std::cout << "Computing Residual from CNumerics" << endl;
    //not printed at each point
    return ResidualType<>(nullptr,nullptr,nullptr);
  }


  //A meaningless function in CNumerics which my real function in flow_sources.hpp can override
  inline virtual ResidualType<> ComputeResidual(const CConfig* config, su2double coords[], su2double magnitude){
    return ResidualType<>(nullptr,nullptr,nullptr);
  }
