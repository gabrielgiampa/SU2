...
CEulerSolver::CEulerSolver(CGeometry *geometry, CConfig *config,
  ...
  if (navier_stokes) {
    ...
    std::cout << "Nodes initialized" << endl;
  }
...
void CEulerSolver::Preprocessing(CGeometry *geometry, CSolver **solver_container, CConfig *config, unsigned short iMesh,
                                 unsigned short iRKStep, unsigned short RunTime_EqSystem, bool Output) {
  //std::cout << "Fine solver preprocessing" << endl;
  //This prints so we know we are using the CEulerSolver Preprocessing
...
void CEulerSolver::Centered_Residual(CGeometry *geometry, CSolver **solver_container, CNumerics **numerics_container,
  ...
  //std::cout << "Found the residual solver" << endl;
  //Yes we have
...
void CEulerSolver::Source_Residual(CGeometry *geometry, CSolver **solver_container,
  ...
  const bool spherical        = config->GetSpherical(); //controls whether or not we use spherical gravity!
  ...
  if (spherical) { //we are not using Cartesian gravity!

    su2double coords[nPointDomain][3]; //holds the coordinates of each point
    su2double mag[nPointDomain]; //holds the sqrt(magnitude)
    ...
    for (iPoint = 0; iPoint < nPointDomain; iPoint++) {
      ...
      for (unsigned short iDim = 0; iDim < 3; iDim ++){
        coords[iPoint][iDim] = geometry -> nodes ->GetCoord(iPoint, iDim);
        mag[iPoint] = mag[iPoint] + pow(coords[iPoint][iDim],2);
      }
      mag[iPoint] = sqrt(mag[iPoint]);
      ...
      auto residual = numerics->ComputeResidual(config,coords[iPoint],mag[iPoint]);
  ...
  if (body_force && !spherical){ //we are using cartesian gravity
    /*--- Loop over all points ---*/
    SU2_OMP_FOR_STAT(omp_chunk_size)
    for (iPoint = 0; iPoint < nPointDomain; iPoint++) {
      ...
      /*--- Compute the rotating frame source residual ---*/
      //std::cout << "about to compute residual" << endl;
      //calls ComputeResidual in flow_sources
      auto residual = numerics->ComputeResidual(config);
...
void CEulerSolver::Source_Template(CGeometry *geometry, CSolver **solver_container, CNumerics *numerics,
  ...
  const su2double grav = -9.8;
  su2double coords[nPointDomain][3];
  su2double mag[nPointDomain];
  SU2_OMP_FOR_DYN(omp_chunk_size)
  //std::cout <<
  for(unsigned long iPoint = 0; iPoint < nPointDomain; iPoint++){
    for (unsigned short iDim = 0; iDim < 3; iDim ++){
      coords[iPoint][iDim] = geometry -> nodes->GetCoord(iPoint, iDim);
      mag[iPoint] = 0.0 + pow(coords[iPoint][iDim],2);
    }
    mag[iPoint] = sqrt(mag[iPoint]);
    mag[iPoint] = grav / mag[iPoint];
  }

  //std::cout << "[ " << coords[30][0] << " " << coords[30][1] << " " << coords[30][2] << " ]" <<endl;
  //std::cout << nPointDomain << endl;
...
void CEulerSolver::Explicit_Iteration(CGeometry *geometry, CSolver **solver_container,
  ...
  //std::cout << "Made it to implicit euler iteration" << endl;
...
void CEulerSolver::ImplicitEuler_Iteration(CGeometry *geometry, CSolver **solver_container, CConfig *config) {
  ...
  //std::cout << "Made it to ImplicitEuler_Iteration" << endl;

  const bool adjoint = config->GetContinuous_Adjoint();
  const bool roe_turkel = config->GetKind_Upwind_Flow() == TURKEL;
  const bool low_mach_prec = config->Low_Mach_Preconditioning();

  //std::cout << "roe_turkel and low_mach_prec" << roe_turkel << low_mach_prec << endl; both zero!
  ...
  SU2_OMP(for schedule(static,omp_chunk_size) nowait)
  for (unsigned long iPoint = 0; iPoint < nPointDomain; iPoint++) {
    //we're cycling through each point, which is fun

    /*--- Read the residual ---*/

    su2double* local_Res_TruncError = nodes->GetResTruncError(iPoint);
    //std::cout << "Made it past local_Res_TruncError" << endl;

    /*--- Read the volume ---*/

    su2double Vol = geometry->nodes->GetVolume(iPoint) + geometry->nodes->GetPeriodicVolume(iPoint);
    //std::cout << "Made it past volume allocation" << endl;

    /*--- Modify matrix diagonal to assure diagonal dominance ---*/

    if (nodes->GetDelta_Time(iPoint) != 0.0) {
      //std::cout << "Annoying..." << endl;
      ...
      if (roe_turkel || low_mach_prec) {
        ...
        //std::cout << "Added Jacobian 2d diag iPoint Delta, here is delta" << Delta << endl;
      }
    }
    else {
    ...
    //std::cout << "Right before we cycle through variables" << endl;
    for (unsigned short iVar = 0; iVar < nVar; iVar++) {
  ...
  //std::cout << "We've cycled through every point" << endl;
  SU2_OMP_CRITICAL
  for (unsigned short iVar = 0; iVar < nVar; iVar++) {
    AddRes_RMS(iVar, resRMS[iVar]);
    AddRes_Max(iVar, resMax[iVar], geometry->nodes->GetGlobalIndex(idxMax[iVar]), coordMax[iVar]);
  ...
  //std::cout << "Right above ghost points" << endl;
  ...
  //std::cout << "Right above freeing the local preconditioner" << endl;
  ...
  //std::cout << "Right before solving / smoothing the linear system" << endl;
  ...
  //std::cout << "Right before computing under relaxation_factor" << endl;

  ComputeUnderRelaxationFactor(solver_container, config);

  /*--- Update solution (system written in terms of increments) ---*/

  //std::cout << "Right before updating solution" << endl;
