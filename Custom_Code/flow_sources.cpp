...
CSourceBase_Flow::CSourceBase_Flow(unsigned short val_nDim, unsigned short val_nVar, const CConfig* config) :
  ...
  for(unsigned short iVar = 0; iVar < nVar; ++iVar)
    ...
    //std::cout << "Jacobian assigned" << endl;
}
...
CNumerics::ResidualType<> CSourceBodyForce::ComputeResidual(const CConfig* config) {
  ...
  //std::cout << "Done the Regular Body Force" << endl;
  return ResidualType<>(residual, jacobian, nullptr);
}


CSphericalBodyForce::CSphericalBodyForce(unsigned short val_nDim, unsigned short val_nVar, const CConfig* config) :
                  CSourceBase_Flow(val_nDim, val_nVar, config) {
  field_strength = abs(config ->GetField_Strength());


}
CNumerics::ResidualType<> CSphericalBodyForce::ComputeResidual(const CConfig* config, su2double coords[], su2double mag) {
  su2double planet_mass = config -> GetPlanet_Mass();
  unsigned short iDim;
  su2double Force_Ref = config->GetForce_Ref(); // this is just the 1
  su2double force;
  if(planet_mass == -1.0){ //we aren't using Newtonian gravity, we opt for uniform acceleration
    force = -1.0 * field_strength / mag;

    //std::cout << "Total const magnitude: " << sqrt(pow(force*coords[0],2) + pow(force*coords[1],2) + pow(force*coords[2],2)) << endl;
    //std::cout << "Standard field: " << force << endl;
    ...
    for (iDim = 0; iDim < nDim; iDim++){
      //residual[iDim+1] = -Volume * U_i[0] * (coords[iDim]*force);
      residual[iDim+1] = -Volume * U_i[0] * (force*(coords[iDim] - config ->GetCenter_of_Mass()[iDim]));
      //std::cout << "Let's be sure... " << coords[iDim] << " " << force*coords[iDim] << endl;
      //works perfectly!
    }
    /*--- Energy contribution ---*/
    ...
    residual[nDim+1] = 0.0;
    for (iDim = 0; iDim < nDim; iDim++){
      //residual[nDim+1] += -Volume * U_i[iDim+1] * (coords[iDim]*force);
      residual[nDim+1] += -Volume * U_i[iDim+1] * (force*(coords[iDim]- config ->GetCenter_of_Mass()[iDim]));
    }
    //std::cout << "Computed the residuals my own way" << endl;
    return ResidualType<>(residual, jacobian, nullptr);
  }
  else { //we're using Newtonian gravity!
    const su2double big_G = 6.674e-11;
    su2double planet_radius = config -> GetPlanet_Radius();
    force = -1.0/mag;
    su2double newton_field = big_G * planet_mass/pow(mag + (planet_radius - 1.5),2) * force;
    //std::cout << "New field strength: " << newton_field << endl;
    //std::cout << "Total acceleration magnitude: " << sqrt(pow(newton_field*coords[0],2) + pow(newton_field*coords[1],2) + pow(newton_field*coords[2],2)) << endl;
    /*--- Zero the continuity contribution ---*/
    ...
    for (iDim = 0; iDim < nDim; iDim++){
      //residual[iDim+1] = -Volume * U_i[0] * (coords[iDim]*force);
      residual[iDim+1] = -Volume * U_i[0] * (newton_field*(coords[iDim] - config ->GetCenter_of_Mass()[iDim]));
      //std::cout << "Let's be sure... " << coords[iDim] << " " << force*coords[iDim] << endl;
      //works perfectly!
    }
    /*--- Energy contribution ---*/
    ...
    for (iDim = 0; iDim < nDim; iDim++){
      //residual[nDim+1] += -Volume * U_i[iDim+1] * (coords[iDim]*force);
      residual[nDim+1] += -Volume * U_i[iDim+1] * (newton_field*(coords[iDim]- config ->GetCenter_of_Mass()[iDim]));
    }
    //std::cout << "Computed the residuals my own way" << endl;
    return ResidualType<>(residual, jacobian, nullptr);
